 ...
 * Patched for T1 iBridge (2016-2017 MacBook Pro) compatibility
 *
 * Copyright (c) 2023 Kerem Karabay <kekrby@gmail.com>
 * T1 patches: Added support for USB product ID 0x8600 and relaxed
 * protocol size checks for T1 iBridge smaller response packets.
 */
...
#define APPLETBDRM_PIXEL_FORMAT     cpu_to_le32(0x52474241) /* RGBA, big-endian */
#define APPLETBDRM_BITS_PER_PIXEL   24
#define APPLETBDRM_BULK_MSG_TIMEOUT 1000

#define APPLETBDRM_MSG_CLEAR_DISPLAY    cpu_to_le32(0x43454c52)
#define APPLETBDRM_MSG_GET_INFORMATION  cpu_to_le32(0x47494e46)
#define APPLETBDRM_MSG_SIGNAL_READINESS cpu_to_le32(0x52454459)
#define APPLETBDRM_MSG_UPDATE_COMPLETE  cpu_to_le32(0x55504443)
#define APPLETBDRM_MSG_UPDATE_COMPLETE_T1 cpu_to_le32(0x5544434c) /* UDCL - T1 variant */
...
#define adev_to_udev(_adev) interface_to_usbdev(to_usb_interface(adev->drm.dev))
...
/* T2 information structure (65 bytes) */
struct appletbdrm_msg_information {
    ...
    __le32 pixel_format;
    __le32 width_inches;
    __le32 height_inches;
} __packed;

/* T1 information structure (52 bytes) - smaller than T2 */
struct appletbdrm_msg_information_t1 {
    struct appletbdrm_msg_response_header header;
    u8 unk_14[12];
    __le32 width;
    __le32 height;
...
struct appletbdrm_device {
    unsigned int in_ep;
    unsigned int out_ep;
    bool is_t1; /* true for T1 iBridge (0x8600) */
...
retry:
    ...
    if (actual_size != size) {
        if (adev->is_t1 && actual_size >= 16) {
            /* T1 iBridge sends shorter responses - accept them */
            drm_dbg(drm, "T1: accepted %d bytes (expected %zu)\n",
                actual_size, size);
        } else {
            drm_err(drm, "Actual size (%d) doesn't match expected size (%zu)\n",
                actual_size, size);
    ...
    /* For T1 with short responses (< 20 bytes), skip msg check entirely */
    if (actual_size >= sizeof(struct appletbdrm_msg_response_header)) {
        if (response->msg != expected_response &&
            !(adev->is_t1 && response->msg == APPLETBDRM_MSG_UPDATE_COMPLETE_T1 &&
              expected_response == APPLETBDRM_MSG_UPDATE_COMPLETE)) {
            drm_err(drm, "Unexpected response from device (expected %p4cl found %p4cl)\n",
                &expected_response, &response->msg);
            return -EIO;
        }
    } else if (!adev->is_t1) {
        drm_err(drm, "Response too short to contain message field (%d bytes)\n", actual_size);
        return -EBADMSG;
    }
...
static int appletbdrm_get_information(struct appletbdrm_device *adev)
{
    ...
    /* Both T1 and T2 have width and height at the same offset */
    adev->width = get_unaligned_le32(&info->width);
    adev->height = get_unaligned_le32(&info->height);

    if (adev->is_t1) {
        /* T1 sends fewer fields; assume 24bpp BGR like T2 */
        drm_info(drm, "T1 iBridge display: %ux%u\n", adev->width, adev->height);
    } else {
        ...
        if (get_unaligned(&info->pixel_format) != APPLETBDRM_PIXEL_FORMAT) {
            __le32 pf = get_unaligned(&info->pixel_format);
                   ...
                   struct drm_plane_state *state)
{
    ...
    /* T1 sends shorter responses without timestamp - skip check for T1 */
    if (!adev->is_t1 && response->timestamp != footer->timestamp) {
                ...
                const struct usb_device_id *id)
{
    ...
    if (ret) {
        dev_dbg(dev, "appletbdrm: Failed to find bulk endpoints\n");
        return ret;
    }
    ...
    adev->is_t1 = (le16_to_cpu(interface_to_usbdev(to_usb_interface(dev))->descriptor.idProduct) == 0x8600);

    drm = &adev->drm;

    if (adev->is_t1)
        drm_info(drm, "T1 iBridge Touch Bar detected\n");
    ...
    if (dma_dev) {
        ...
        drm_warn(drm, "buffer sharing not supported");
    }
